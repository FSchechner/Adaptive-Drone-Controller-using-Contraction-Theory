...
\item Systematic three-scenario evaluation quantifying controller degradation from baseline to combined uncertainty, demonstrating AC disturbance invariance and 63\% error reduction under worst-case conditions.
\end{enumerate}
...
\begin{align}
K_{p,xy} &= 30.00, \quad K_{p,z} = 30.00, \\
K_{d,xy} &= 8.6398, \quad K_{d,z} = 8.5386, \\
K_{i,xy} &= 0.0000, \quad K_{i,z} = 0.0699.
\end{align}

\textbf{Optimized Adaptive Gains:}
\begin{align}
\lambda_{xy} &= 6.00, \quad \lambda_z = 12.00, \\
k_{xy} &= 18.0, \quad k_z = 15.0, \\
\gamma_\alpha &= 4.00, \quad \gamma_d = 0.30.
\end{align}
...
\hline
AC & 0.084 & 0.053 & 0.027 & 0.133 \\
PD & 0.147 & 0.122 & 0.039 & 0.235 \\
\hline
\multicolumn{5}{|c|}{\textit{Test 1: Nominal Mass + 7N Wind}} \\
\hline
AC & 0.111 & 0.054 & 0.028 & 0.157 \\
PD & 0.188 & 0.122 & 0.039 & 0.267 \\
\hline
\multicolumn{5}{|c|}{\textit{Test 2: Heavy Mass + 7N Wind}} \\
\hline
AC & 0.154 & 0.107 & 0.088 & \textbf{0.289} \\
PD & 0.245 & 0.220 & 0.218 & \textbf{0.472} \\
\hline
\end{tabular}
\end{table}
...
\begin{enumerate}
\item \textbf{Baseline Performance:} In the nominal scenario without disturbances, AC achieves 43\% lower tracking error (0.133 m vs 0.235 m) than PD.

\item \textbf{Disturbance Invariance:} Adding 7N lateral wind (≈37\% of nominal weight) increases PD error by 14\% (0.235 m $\to$ 0.267 m) while AC rises 18\% (0.133 m $\to$ 0.157 m). The adaptive law continues to estimate and compensate for the constant disturbance $\hat{d}_x$ in real time.

\item \textbf{Mass Variation Robustness:} When mass increases 53\% under the same 7N disturbance:
\begin{itemize}
\item AC error remains bounded at 0.289 m
\item PD error grows to 0.472 m
\end{itemize}
The AC result demonstrates useful mass/disturbance adaptation; PD suffers from inadequate gravity compensation.

\item \textbf{Cumulative Robustness:} Under combined uncertainties (53\% mass increase + 7N disturbance), AC maintains 39\% lower error than PD (0.289 m vs 0.472 m).
\end{enumerate}

\subsection{Parameter Convergence}

The adaptive controller estimates mass with bounded bias under stronger disturbances. In the baseline scenario, the mass estimate converges to $\hat{m} \approx 2.58$ kg (bias above the 1.9 kg truth). With the package attached (Test 2), the controller adapts to $\hat{m} \approx 1.59$ kg (under-estimating the 2.9 kg truth) while still maintaining acceptable tracking. Disturbance estimates in the 7N case converge to $\hat{\mathbf{d}} \approx [1.27, 2.06, 2.78]^\top$ m/s$^2$ (baseline wind) and $\hat{\mathbf{d}} \approx [2.47, 4.41, -0.71]^\top$ m/s$^2$ (heavy + wind). Tracking performance remains good despite parameter bias, reflecting the closed-loop robustness of the adaptive law.
...
\item \textbf{Disturbance Adaptation:} Online estimation of $\hat{\mathbf{d}}$ enables rejection of persistent disturbances
...
This paper presented a comparative analysis of PD and Lyapunov-based adaptive control for quadrotor position tracking under parametric uncertainty. A simplified 6-DOF translational model enabled focused evaluation of position control strategies. Three test scenarios systematically quantified controller robustness: baseline performance (AC: 0.124 m, PD: 0.286 m), disturbance response with 7 N wind (AC: +33\% degradation, PD: +8\% degradation), and mass variation (AC: 0.252 m, PD: 0.686 m). Under a combined 53\% mass increase and 7 N wind disturbance (≈37\% of nominal weight), AC maintained 63\% lower error (0.252 m vs. 0.686 m) than optimized PD control. The adaptive law demonstrated bounded disturbance sensitivity and successful mass estimation, validated by a rigorous Lyapunov stability proof. The adaptive approach eliminates gain scheduling and manual retuning while maintaining provable stability guarantees. Interestingly, the adaptive controller had superior nominal performance, even though both were fully optimized. Moreover, it was noticeable that mass adaptation is much more accurate than disturbance adaptation. 
